...
### Kepler GPUs (Custom Build Required)

Kepler-architecture GPUs (compute capability 3.5 and 3.7) can be supported with a custom build using CUDA 11.x and GCC 10:

| Compute Capability | Family              | Cards                                                                                      |
| ------------------ | ------------------- | ------------------------------------------------------------------------------------------ |
| 3.7                | Tesla               | `K80`                                                                                      |
| 3.5                | Tesla               | `K40` `K20` `K20X`                                                                         |
|                    | GeForce GTX         | `GTX TITAN` `GTX TITAN Black` `GTX TITAN Z` `GTX 780 Ti` `GTX 780`                        |

> **Note:** Kepler GPUs require building from source with the `CUDA 11 Kepler` preset. 
> Performance will be significantly slower than newer GPUs due to lack of tensor cores and FP16 hardware support.
> See the [development docs](./development#linux-cuda-nvidia) for build instructions.
